            ...
            if(HidD_GetPreparsedData(ht,&ppd)) {
                ...
                if(HidP_GetCaps(ppd,&caps)==HIDP_STATUS_SUCCESS) {
                    if(caps.InputReportByteLength>bestSize) {
                        ...
                        strncpy_s(bestPath,sizeof(bestPath),det->DevicePath,sizeof(bestPath)-1);
                    }
                }
...
int main() {
    ...
    while(!hNacon.valid()){
        ...
        if(!hNacon.valid()){uiMsg("Waiting for Nacon MG-X — plug in the gamepad...",CC_YEL);Sleep(1500);}
    }
    uiClearMsg();
